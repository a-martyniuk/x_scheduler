...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({ posts, globalStats, accounts, onSync }) => {
    ...
    const getLastSynced = (dateStr?: string) => {
        if (!dateStr) return null;
        // Ensure UTC interpretation if no timezone offset is present
        const cleanDate = dateStr.endsWith('Z') || dateStr.includes('+') ? dateStr : `${dateStr}Z`;
        return new Date(cleanDate).toLocaleString('es-AR', { timeZone: 'America/Argentina/Buenos_Aires' });
    };
